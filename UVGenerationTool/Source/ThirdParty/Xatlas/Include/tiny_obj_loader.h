...
#define IS_NEW_LINE(x) (((x) == '\r') || ((x) == '\n') || ((x) == '\0'))
	...
	bool ExportFace(std::vector<shape_t>& shapes,       // [output]
				std::vector<float> v,
				std::vector<float> vn,
				...
				unsigned int flags)
	{
		...
		{
			std::string temp = std::to_string(index[i]) + "//" + std::to_string(index[i]) + " "
				+ std::to_string(index[i + 1]) + "//" + std::to_string(index[i + 1]) + " "
				+ std::to_string(index[i + 2]) + "//" + std::to_string(index[i + 2]);
